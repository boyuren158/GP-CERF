...
Existing R packages for estimating causal exposure-response functions of continuous exposures usually requires resampling approaches, such as boostrap, to obtain uncertainty of the estimates [e.g., @CausalGPS_R]. However, when the number of observations is large, resampling based algorithms can be computationally expensive. To address this gap, we have implemented a novel Bayesian approach that uses a Gaussian Processes (GPs) prior for the counterfactual outcome surfaces to enable flexible estimation of the CERF. Numerous R packages exist for conducting Gaussian process regression [e.g., @GauPro_2023, @mlegp_2022, and @GPfit_2019]. We opted not to use these packages primarily because they employ conventional methods for hyper-parameter tuning. These methods either sample from the posterior distributions of the hyper-parameters or maximize the marginal likelihood function. In contrast, the tuning method we introduced in our paper focuses on achieving optimal covariate balancing. By leveraging the posterior distributions of the model parameters, we can automatically quantify the uncertainty of the estimated CERF [for more details see @Ren_2021_bayesian]. Since standard GPs are notorious for their lack of scalability due to the operations involving inversion of the covariance matrices, we consider a nearest-neighbour GP (nnGP) prior to achieve computationally efficient inference of CERF when dealing with large-scale datasets.
